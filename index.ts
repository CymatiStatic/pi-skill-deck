 ...
 *
 * Merged from SkyNotion/pi-skill-manager fork (category gating, custom skill
 * paths, per-session overrides) on top of the original pi-skill-deck.
 */
...
const state: ManagerState = {
  queuedSkill: null,
  // Start with all enabled ({} → enabledCategories undefined → null → all enabled).
  // reloadConfig() replaces this on session_start; this initial value only guards
  // the window before the config file is read so nothing is hidden prematurely.
  config: {},
...
export default function piSkillManager(pi: ExtensionAPI): void {
  ...
  // ── Shared overlay handler used by all command aliases ──
  const openManagerOverlay = async (ctx: ExtensionContext): Promise<void> => {
    const allSkills = loadAllSkills();
  ...
  pi.registerCommand("skill-manager", {
    ...
    handler: async (_args: string, ctx: ExtensionContext) => openManagerOverlay(ctx),
  });

  // ── Register /skill-deck alias (original command name, preserved) ──
  pi.registerCommand("skill-deck", {
    description: "Alias for /skill-manager — open the skill browser",
    handler: async (_args: string, ctx: ExtensionContext) => openManagerOverlay(ctx),
  });
