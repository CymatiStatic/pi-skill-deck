...
const DEFAULT_CONFIG: SkillManagerConfig = {
  // enabledCategories intentionally omitted → absent = ALL enabled (non-destructive default).
  // A fresh install must NOT silently hide every skill from Pi. Users opt IN to gating
  // by toggling categories in the overlay, which writes an explicit enabledCategories list.
  customSkills: [],
};
